...
export const STARKNET_NETWORK: SupportedNetwork =
  (import.meta.env.VITE_STARKNET_NETWORK as SupportedNetwork) ||
  DEFAULT_NETWORK;
...
const validatorList = Object.values(validatorPresets) as {
  name: string;
  stakerAddress: string;
}[];

export const stakingValidators: StakingValidator[] = validatorList.map((validator) => ({
  name: validator.name,
